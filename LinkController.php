...
class LinkAdmin { // classe administrativa => /qrlink/admin
    ...
    public function apagarLINKS() { ///// Apagar TODOS os links da tabela
        ...
        } else  echo json_encode( [ "status" => "delete" ]);
        exit();
    } // apagarLINKS

    public function apagarItem($REQUEST) { // Apagar APENAS UM link da tabela
        ...
        foreach ($arr as $row) {
            ...
            $stmt = $this->conexao->prepare($query);
            $stmt->bindParam(':short_code', $row['short_code'], PDO::PARAM_STR);
            $sqlResult = $stmt->execute();

            header("Content-type: application/json");
            if (!$sqlResult) { // Apenas retornará erro ao tentar apagar, não ao executar o SELECT no banco
                    echo json_encode( [ "status" => "error" ]);
            } else  echo json_encode( [ "status" => "delete" ]);
            exit();
        }
